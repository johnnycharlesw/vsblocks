...
export async function wrapWithAbbreviation(args: any): Promise<boolean> {
	...
	// Also helps with https://github.com/johnnycharlesw/vsblocks/issues/113930 by avoiding `editor.linkedEditing`
...
export function expandEmmetAbbreviation(args: any): Thenable<boolean | undefined> {
	...
	selectionsInReverseOrder.forEach(selection => {
		...
		if (isStyleSheet(syntax) && abbreviation.endsWith(':')) {
			// Fix for https://github.com/johnnycharlesw/vsblocks/issues/1623
			return;
		}
...
export function isValidLocationForEmmetAbbreviation(document: vscode.TextDocument, rootNode: Node | undefined, currentNode: Node | undefined, syntax: string, offset: number, abbreviationRange: vscode.Range): boolean {
	if (isStyleSheet(syntax)) {
		...
		// Fixes https://github.com/johnnycharlesw/vsblocks/issues/74505
		...
		// Fix for https://github.com/johnnycharlesw/vsblocks/issues/34162
		...
		// Workaround for https://github.com/johnnycharlesw/vsblocks/30188
	...
	if (currentHtmlNode) {
		...
		// Fix for https://github.com/johnnycharlesw/vsblocks/issues/28829
		if (!currentHtmlNode.open || !currentHtmlNode.close ||
			!(currentHtmlNode.open.end <= offset && offset <= currentHtmlNode.close.start)) {
			return false;
		}

		// Fix for https://github.com/johnnycharlesw/vsblocks/issues/35128
	...
	while (i >= 0) {
		...
		// Fix for https://github.com/johnnycharlesw/vsblocks/issues/55411
...
function expandAbbr(input: ExpandAbbreviationInput): string | undefined {
	...
	if (input.textToWrap) {
		...
		if (expandOptions.options) {
			// Below fixes https://github.com/johnnycharlesw/vsblocks/issues/29898
