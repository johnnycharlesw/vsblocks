...
if [ -d "$VSCODE_CLIENT_SYSROOT_DIR" ]; then
  ...
  SYSROOT_ARCH="$SYSROOT_ARCH" VSCODE_SYSROOT_DIR="$VSCODE_CLIENT_SYSROOT_DIR" node -e '(async () => { const { getVSBlocksSysroot } = require("./build/linux/debian/install-sysroot.js"); await getVSBlocksSysroot(process.env["SYSROOT_ARCH"]); })()'
fi
...
if [ -d "$VSCODE_REMOTE_SYSROOT_DIR" ]; then
  ...
  SYSROOT_ARCH="$SYSROOT_ARCH" VSCODE_SYSROOT_DIR="$VSCODE_REMOTE_SYSROOT_DIR" VSCODE_SYSROOT_PREFIX="-glibc-2.28-gcc-8.5.0" node -e '(async () => { const { getVSBlocksSysroot } = require("./build/linux/debian/install-sysroot.js"); await getVSBlocksSysroot(process.env["SYSROOT_ARCH"]); })()'
fi
