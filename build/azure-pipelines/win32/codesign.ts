...
async function main() {
	...
	if (process.env['BUILT_CLIENT']) {
		// Product version
		const version = await $`node -p "require('../VSBlocks-win32-${arch}/resources/app/package.json').version"`;

		printBanner('Package client');
		const clientArchivePath = `.build/win32-${arch}/VSBlocks-win32-${arch}-${version}.zip`;
		await $`7z.exe a -tzip ${clientArchivePath} ../VSBlocks-win32-${arch}/* "-xr!CodeSignSummary*.md"`.pipe(process.stdout);
