...
async function main(buildDir?: string): Promise<void> {
	...
	const appRoot = path.join(buildDir, `VSBlocks-darwin-${arch}`);
	...
	if (arch !== 'universal') {
		await spawn('plutil', [
			...
			'-string',
			'An application in VSBlocks wants to use AppleScript.',
			`${infoPlistPath}`
		]);
		await spawn('plutil', [
			'-replace',
			...
			'-string',
			'An application in VSBlocks wants to use the Microphone.',
			`${infoPlistPath}`
		]);
		await spawn('plutil', [
			'-replace',
			...
			'-string',
			'An application in VSBlocks wants to use the Camera.',
			`${infoPlistPath}`
		]);
	}
