...
function getVSBlocksSysrootChecksum(expectedName: string) {
...
async function fetchUrl(options: IFetchOptions, retries = 10, retryDelay = 1000): Promise<undefined> {
	try {
		...
		try {
			const response = await fetch(`https://api.github.com/repos/johnnycharlesw/vsblocks-linux-build-agent/releases/tags/v${version}`, {
			...
			if (response.ok && (response.status >= 200 && response.status < 300)) {
				...
				if (!asset) {
					throw new Error(`Could not find asset in release of johnnycharlesw/vsblocks-linux-build-agent @ ${version}`);
				}
...
export async function getVSBlocksSysroot(arch: DebianArchString, isMusl: boolean = false): Promise<string> {
	...
	const checksumSha256 = getVSBlocksSysrootChecksum(expectedName);
	if (!checksumSha256) {
