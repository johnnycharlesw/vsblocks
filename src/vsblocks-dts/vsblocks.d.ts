...
declare module 'vsblocks' {
	...
	export class RelativePattern {
		 ...
		 * Example:
		 * ```ts
		 * const folder = vsblocks.workspace.workspaceFolders?.[0];
		 * if (folder) {
		 *
		 *   // Match any TypeScript file in the root of this workspace folder
		 *   const pattern1 = new vsblocks.RelativePattern(folder, '*.ts');
		 *
		 *   // Match any TypeScript file in `someFolder` inside this workspace folder
		 *   const pattern2 = new vsblocks.RelativePattern(folder, 'someFolder/*.ts');
		 * }
		 * ```
		 *
	...
	export class MarkdownString {
		 ...
		 *
		 * ```ts
		 * const md = new vsblocks.MarkdownString(`[link](./file.js)`);
		 * md.baseUri = vsblocks.Uri.file('/path/to/dir/');
		 ...
		 * ```ts
		 * const md = new vsblocks.MarkdownString(`[link](./file.js)`);
		 * md.baseUri = vsblocks.Uri.file('/path/to/otherFile.js');
	 ...
	 * const config = workspace.getConfiguration('launch', vsblocks.workspace.workspaceFolders[0].uri);
	 *
	 * // retrieve values
	...
	export interface Terminal {
		 ...
		 *   	vsblocks.window.showInformationMessage(`Exit code: ${t.exitStatus.code}`);
		 *   }
		 * });
		 * ```
		 */
	...
	export interface ExtensionContext {
		 ...
		 * {@linkcode ExtensionContext.extensionUri extensionUri}, e.g. `vsblocks.Uri.joinPath(context.extensionUri, relativePath);`
		 *
	 ...
	 * a file or folder doesn't exist, use them like so: `throw vsblocks.FileSystemError.FileNotFound(someUri);`
	 */
	...
	export interface Webview {
		 ...
		 * ```html
		 * <script>
		 *     const vsblocks = acquireVsCodeApi(); // acquireVsCodeApi can only be invoked once
		 *     vsblocks.postMessage({ message: 'hello!' });
		 * </script>
		 * ```
		 *
		 ...
		 * setting a [content security policy](https://aka.ms/vsblocks-api-webview-csp).
		 */
		html: string;
		 ...
		 *
		 *   For older versions of vsblocks, if an `ArrayBuffer` is included in `message`,
		 ...
		 *
		 *   However if your extension targets vsblocks 1.57+ in the `engines` field of its
		 ...
		 *
		 * ```ts
		 * webview.html = `<img src="${webview.asWebviewUri(vsblocks.Uri.file('/Users/codey/workspace/cat.gif'))}">`
		 * ```
		 */
	...
	/**
	 * Restore webview panels that have been persisted when vsblocks shuts down.
	 *
	 ...
	 * ```js
	 * // Within the webview
	 * const vsblocks = acquireVsCodeApi();
	 *
	 * // Get existing state
	 * const oldState = vsblocks.getState() || { value: 0 };
	 *
	 * // Update state
	...
	export namespace env {
		...
		/**
		 * The application name of the editor, like 'VSBlocks'.
		 */
		 ...
		 * * VSBlocks itself (`vsblocks:` from `vsblocks.env.uriScheme`)
		 *
		 ...
		 * ```typescript
		 * vsblocks.window.registerUriHandler({
		 *   handleUri(uri: vsblocks.Uri): vsblocks.ProviderResult<void> {
		 ...
		 * });
		 *
		 * const callableUri = await vsblocks.env.asExternalUri(vsblocks.Uri.parse(vsblocks.env.uriScheme + '://my.extension/did-authenticate'));
		 * await vsblocks.env.openExternal(callableUri);
		 * ```
		 *
	...
	export namespace window {
		 ...
		 * the following rules:
		 *
		 * - The uri-scheme must be `vsblocks.env.uriScheme`;
	...
	export class TreeItem {
		 ...
		 *
		 * Please use `vsblocks.open` or `vsblocks.diff` as command IDs when the tree item is opening
	...
	export interface Pseudoterminal {
		 ...
		 * ```typescript
		 * const writeEmitter = new vsblocks.EventEmitter<string>();
		 * const pty: vsblocks.Pseudoterminal = {
		 *   onDidWrite: writeEmitter.event,
		 *   open: () => writeEmitter.fire('\x1b[31mHello world\x1b[0m'),
		 *   close: () => {}
		 * };
		 * vsblocks.window.createTerminal({ name: 'My terminal', pty });
		 * ```
		 *
		 ...
		 * ```typescript
		 * const dimensionsEmitter = new vsblocks.EventEmitter<vsblocks.TerminalDimensions>();
		 * const pty: vsblocks.Pseudoterminal = {
		 *   onDidWrite: writeEmitter.event,
		 ...
		 * ```typescript
		 * const writeEmitter = new vsblocks.EventEmitter<string>();
		 * const closeEmitter = new vsblocks.EventEmitter<void>();
		 * const pty: vsblocks.Pseudoterminal = {
		 *   onDidWrite: writeEmitter.event,
		 ...
		 *   handleInput: data => {
		 *     if (data !== 'y') {
		 *       vsblocks.window.showInformationMessage('Something went wrong');
		 *     }
		 *     closeEmitter.fire();
		 *   }
		 * };
		 * const terminal = vsblocks.window.createTerminal({ name: 'Exit example', pty });
		 * terminal.show(true);
		 * ```
		 */
		 ...
		 * ```typescript
		 * const writeEmitter = new vsblocks.EventEmitter<string>();
		 * const changeNameEmitter = new vsblocks.EventEmitter<string>();
		 * const pty: vsblocks.Pseudoterminal = {
		 *   onDidWrite: writeEmitter.event,
		 ...
		 * };
		 * vsblocks.window.createTerminal({ name: 'Local echo', pty });
		 * ```
		 */
	...
	export namespace workspace {
		 ...
		 * files, e.g. `vsblocks.workspace.fs.readDirectory(someUri)` allows to retrieve all entries
		 * of a directory or `vsblocks.workspace.fs.stat(anotherUri)` returns the meta data for a
		 * file.
		 */
		 ...
		 *
		 * The location can e.g. be used with the `vsblocks.openFolder` command to
		 * open the workspace again after it has been closed.
		 *
		 * **Example:**
		 * ```typescript
		 * vsblocks.commands.executeCommand('vsblocks.openFolder', uriOfWorkspace);
		 * ```
		 *
		 * Refer to https://code.visualstudio.com/docs/editor/workspaces for more information on
		 * the concept of workspaces.
		 ...
		 * by an optional set of `workspaceFoldersToAdd` on the `vsblocks.workspace.workspaceFolders` array. This "splice"
		 ...
		 *
		 * ```ts
		 * const watcher = vsblocks.workspace.createFileSystemWatcher(new vsblocks.RelativePattern(<folder>, <pattern>));
		 *
		 ...
		 *
		 * ```ts
		 * vsblocks.workspace.createFileSystemWatcher(new vsblocks.RelativePattern(vsblocks.workspace.workspaceFolders[0], '**​/*.js'));
		 * ```
		 *
		 * If you want to monitor file events across all opened workspace folders:
		 *
		 * ```ts
		 * vsblocks.workspace.createFileSystemWatcher('**​/*.js');
		 * ```
		 *
		 ...
		 *
		 * ```ts
		 * vsblocks.workspace.createFileSystemWatcher(new vsblocks.RelativePattern(vsblocks.Uri.file(<path to folder outside workspace>), '*.js'));
		 * ```
		 *
		 ...
		 *
		 * ```ts
		 * vsblocks.workspace.createFileSystemWatcher(new vsblocks.RelativePattern(vsblocks.window.activeTextEditor.document.uri, '*'));
		 * ```
		 *
	...
	export namespace languages {
		 ...
		 * doc.uri; // `vsblocks-notebook-cell:///my/notebook.ipynb#gl65s2pmha`;
	...
	export class NotebookCellOutput {
		 ...
		 *
		 * ```ts
		 * new vsblocks.NotebookCellOutput([
		 * 	vsblocks.NotebookCellOutputItem.text('Hello', 'text/plain'),
		 * 	vsblocks.NotebookCellOutputItem.text('<i>Hello</i>', 'text/html'),
		 * 	vsblocks.NotebookCellOutputItem.text('_Hello_', 'text/markdown'),
		 * 	vsblocks.NotebookCellOutputItem.text('Hey', 'text/plain'), // INVALID: repeated type, editor will pick just one
		 * ])
		 * ```
		 */
	...
	export namespace debug {
		 ...
		 * The named configurations are looked up in '.vsblocks/launch.json' found in the given folder.
	 ...
	 *
	 * ```javascript
	 * export function activate(context: vsblocks.ExtensionContext) {
	 * 	let api = {
	 * 		sum(a, b) {
	 * 			return a + b;
	 * 		},
	 ...
	 * [vsblocks-l10n repo](https://github.com/johnnycharlesw/vsblocks-l10n).
	 *
	 ...
	 *
	 * ```js
	 * const sender: vsblocks.TelemetrySender = {...};
	 * const logger = vsblocks.env.createTelemetryLogger(sender);
	 *
	 ...
	 * failure causes, like `if(someError.code === vsblocks.LanguageModelError.NotFound.name) {...}`
	...
	export namespace lm {
		 ...
		 *
		 * ```ts
		 * const models = await vsblocks.lm.selectChatModels({ family: 'gpt-3.5-turbo' });
		 * if (models.length > 0) {
		 * 	const [first] = models;
		 ...
		 * {@link LanguageModelPromptTsxPart prompt-tsx}-parts. If the tool caller is using `@vsblocks/prompt-tsx`, it can
	...
	/**
	 * A language model response part containing a PromptElementJSON from `@vsblocks/prompt-tsx`.
	...
	export class LanguageModelPromptTsxPart {
		 ...
		 * @param value The value of the part, the result of `renderElementJSON` from `@vsblocks/prompt-tsx`.
		 */
		constructor(value: unknown);
	}

	/**
	 * A result returned from a tool invocation. If using `@vsblocks/prompt-tsx`, this result may be rendered using a `ToolResult`.
	 */
