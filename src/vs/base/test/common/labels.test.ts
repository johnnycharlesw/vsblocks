...
suite('Labels', () => {
	...
	test('template', () => {
		...
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: '', appName: 'VSBlocks', separator: { label: ' - ' } }), 'VSBlocks');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: 'Untitled-1', rootName: '', appName: 'VSBlocks', separator: { label: ' - ' } }), 'Untitled-1 - VSBlocks');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: 'monaco', appName: 'VSBlocks', separator: { label: ' - ' } }), 'monaco - VSBlocks');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: 'somefile.txt', rootName: 'monaco', appName: 'VSBlocks', separator: { label: ' - ' } }), 'somefile.txt - monaco - VSBlocks');
		assert.strictEqual(labels.template(t, { dirty: '* ', activeEditorShort: 'somefile.txt', rootName: 'monaco', appName: 'VSBlocks', separator: { label: ' - ' } }), '* somefile.txt - monaco - VSBlocks');

		// real world example (other)
		t = '${dirty}${activeEditorShort}${separator}${rootNameShort}${separator}${appName}';
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: 'monaco (Workspace)', rootNameShort: 'monaco', appName: 'VSBlocks', separator: { label: ' - ' } }), 'monaco - VSBlocks');
	});
