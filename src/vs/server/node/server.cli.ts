...
/*
 * Implements a standalone CLI app that opens VSBlocks from a remote terminal.
 ...
 *  - In external terminals for WSL this calls VSBlocks on the Windows side.
 *    The VSBlocks desktop executable path is passed in env VSCODE_CLIENT_COMMAND.
 */
...
export async function main(desc: ProductDescription, args: string[]): Promise<void> {
	if (!cliPipe && !cliCommand) {
		console.log('Command is only available in WSL or inside a VSBlocks terminal.');
		return;
	}
...
function fatal(message: string, err: any): void {
	console.error('Unable to connect to VSBlocks server: ' + message);
	console.error(err);
	process.exit(1);
}
