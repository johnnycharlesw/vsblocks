...
export class OpenEditorsView extends ViewPane {
	...
	private openEditor(element: OpenEditor, options: { preserveFocus?: boolean; pinned?: boolean; sideBySide?: boolean }): void {
		if (element) {
			...
			const preserveActivateGroup = options.sideBySide && options.preserveFocus; // needed for https://github.com/johnnycharlesw/vsblocks/issues/42399
			if (!preserveActivateGroup) {
				this.editorGroupService.activateGroup(element.group); // needed for https://github.com/johnnycharlesw/vsblocks/issues/6672
			}
