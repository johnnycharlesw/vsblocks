...
export function handleANSIOutput(text: string, linkDetector: ILinkDetector, workspaceFolder: WorkspaceInterfaceFolder | undefined, highlights: IHighlight[] | undefined): HTMLSpanElement {
	...
	function set8BitColor(styleCodes: number[], colorType: 'foreground' | 'background' | 'underline'): void {
		...
		if (color) {
			...
			if (colorType === 'underline') {
				...
				changeColor(colorType, `--vsblocks-debug-ansi-${colorName}`);
				return;
			}
	...
	function setBasicColor(styleCode: number): void {
		...
		if (colorIndex !== undefined && colorType) {
			...
			changeColor(colorType, `--vsblocks-debug-ansi-${colorName.replaceAll('.', '-')}`);
		}
	}
}
	...
	for (const { selector, bg } of areas) {
		const content = ansiColorIdentifiers
			.map(color => {
				...
				return `--vsblocks-debug-ansi-${color.replaceAll('.', '-')}:${bg ? bg.ensureConstrast(actual, 4) : actual}`;
			})
			.filter(isDefined);
