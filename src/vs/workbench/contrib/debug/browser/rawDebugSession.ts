 ...
 */
interface ILaunchVSBlocksArgument {
	prefix?: string;
	path?: string;
}

interface ILaunchVSBlocksArguments {
	args: ILaunchVSBlocksArgument[];
...
export class RawDebugSession implements IDisposable {
	...
	private async dispatchRequest(request: DebugProtocol.Request): Promise<void> {
		...
		if (request.command === 'launchVSBlocks') {
			try {
				let result = await this.launchVsCode(<ILaunchVSBlocksArguments>request.arguments);
				if (!result.success) {
					...
					if (confirmed) {
						result = await this.launchVsCode(<ILaunchVSBlocksArguments>request.arguments);
					} else {
	...
	private launchVsCode(vscodeArgs: ILaunchVSBlocksArguments): Promise<IOpenExtensionWindowResult> {
