...
export class ExtHostCommands implements ExtHostCommandsShape {
			...
			},
			{
				processArgument(arg) {
					return cloneAndChange(arg, function (obj) {
						// Reverse of https://github.com/johnnycharlesw/vsblocks/blob/1f28c5fc681f4c01226460b6d1c7e91b8acb4a5b/src/vs/workbench/api/node/extHostCommands.ts#L112-L127
	...
	private _reportTelemetry(command: CommandHandler, id: string, duration: number) {
		...
		if (id.startsWith('code.copilot.logStructured')) {
			// This command is very active. See https://github.com/johnnycharlesw/vsblocks/issues/254153.
			return;
		}
...
export class ApiCommandArgument<V, O = V> {
	...
	static readonly TestItem = new ApiCommandArgument('testItem', 'A VSBlocks TestItem', v => v instanceof TestItemImpl, extHostTypeConverter.TestItem.from);
	static readonly TestProfile = new ApiCommandArgument('testProfile', 'A VSBlocks test profile', v => v instanceof extHostTypes.TestRunProfileBase, extHostTypeConverter.TestRunProfile.from);
