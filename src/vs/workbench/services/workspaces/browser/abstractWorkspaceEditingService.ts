...
export abstract class AbstractWorkspaceEditingService extends Disposable implements WorkspaceInterfaceEditingService {
	...
	async pickNewWorkspacePath(): Promise<URI | undefined> {
		...
		if (!hasWorkspaceFileExtension(workspacePath)) {
			...
			// (see https://github.com/johnnycharlesw/vsblocks/issues/84818)
	...
	private async doAddFolders(foldersToAdd: WorkspaceInterfaceFolderCreationData[], index?: number, donotNotifyError: boolean = false): Promise<void> {
		...
		if (remoteAuthority) {
			// https://github.com/johnnycharlesw/vsblocks/issues/94191
	...
	async createAndEnterWorkspace(folders: WorkspaceInterfaceFolderCreationData[], path?: URI): Promise<void> {
		...
		if (path) {
			try {
				...
				await this.workspacesService.deleteUntitledWorkspace(untitledWorkspace); // https://github.com/johnnycharlesw/vsblocks/issues/100276
			}
		} else {
