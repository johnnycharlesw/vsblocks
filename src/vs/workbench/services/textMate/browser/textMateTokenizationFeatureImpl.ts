...
export class TextMateTokenizationFeature extends Disposable implements ITextMateTokenizationService {
		...
		if (this._debugMode) {
			this._progressService.withProgress(
				...
				(progress) => {
					...
					return this._getVSBlocksOniguruma().then((vscodeOniguruma) => {
				...
				(choice) => {
					this._getVSBlocksOniguruma().then((vscodeOniguruma) => {
						this._debugModePrintFunc = () => { };
						this._debugMode = false;
	...
	private async _getOrCreateGrammarFactory(): Promise<TMGrammarFactory> {
		...
		const [vscodeTextmate, vscodeOniguruma] = await Promise.all([importAMDNodeModule<typeof import('vscode-textmate')>('vscode-textmate', 'release/main.js'), this._getVSBlocksOniguruma()]);
	...
	private _getVSBlocksOniguruma(): Promise<typeof import('vscode-oniguruma')> {
		if (!this._vscodeOniguruma) {
			this._vscodeOniguruma = (async () => {
				const [vscodeOniguruma, wasm] = await Promise.all([importAMDNodeModule<typeof import('vscode-oniguruma')>('vscode-oniguruma', 'release/main.js'), this._loadVSBlocksOnigurumaWASM()]);
	...
	private async _loadVSBlocksOnigurumaWASM(): Promise<Response | ArrayBuffer> {
		if (isWeb) {
