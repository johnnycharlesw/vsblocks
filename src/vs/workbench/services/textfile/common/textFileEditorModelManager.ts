...
export class TextFileEditorModelManager extends Disposable implements ITextFileEditorModelManager {
				...
				e.waitUntil((async () => {
					...
					if (modelsToRestore) {
						...
						await Promises.settled(modelsToRestore.map(async modelToRestore => {
							...
							// https://github.com/johnnycharlesw/vsblocks/issues/211374
							...
							if (modelToRestore.language?.id && modelToRestore.language.id !== PLAINTEXT_LANGUAGE_ID) {
								...
								// (https://github.com/johnnycharlesw/vsblocks/issues/203648)
								...
								// (https://github.com/johnnycharlesw/vsblocks/issues/125795)
	...
	private async doResolve(resourceOrModel: URI | TextFileEditorModel, options?: ITextFileEditorModelResolveOrCreateOptions): Promise<TextFileEditorModel> {
		...
		try {
			...
			// https://github.com/johnnycharlesw/vsblocks/issues/138850
			if (didCreateModel) {
				model.dispose();
			}
