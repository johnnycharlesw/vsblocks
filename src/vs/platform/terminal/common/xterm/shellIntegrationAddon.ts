...
export const enum ShellIntegrationOscPs {
	...
	FinalTerm = 133,
	/**
	 * Sequences pioneered by VSBlocks. The number is derived from the least significant digit of
...
/**
 * VSBlocks-specific shell integration sequences. Some of these are based on more common alternatives
 ...
 * the terminal. If multiple shell integration scripts run, VSBlocks will prioritize the VS
 * Code-specific ones.
 *
 * It's recommended that authors of shell integration scripts use the common sequences (`133`)
 * when building general purpose scripts and the VSBlocks-specific (`633`) when targeting only VS
 * Code or when there are no other alternatives (eg. {@link CommandLine `633 ; E`}). These sequences
 * support mix-and-matching.
 */
const enum VSBlocksOscPt {
	/**
	 * The start of the prompt, this is expected to always appear at the start of a line.
	 *
	 * Format: `OSC 633 ; A ST`
	...
	/**
	 * Set the value of an arbitrary property, only known properties will be handled by VSBlocks.
	 *
	 ...
	 *   VSBlocks knows the command line when the execution begins.
	 *
...
export class ShellIntegrationAddon extends Disposable implements IShellIntegration, ITerminalAddon {
	...
	activate(xterm: Terminal) {
		...
		this._register(xterm.parser.registerOscHandler(ShellIntegrationOscPs.VSBlocks, data => this._handleVSBlocksSequence(data)));
	...
	private _doHandleFinalTermSequence(data: string): boolean {
		...
		// It was considered to disable the common protocol in order to not confuse the VSBlocks
	...
	private _handleVSBlocksSequence(data: string): boolean {
		const didHandle = this._doHandleVSBlocksSequence(data);
		...
		if (this._status !== ShellIntegrationStatus.VSBlocks) {
			this._status = ShellIntegrationStatus.VSBlocks;
			this._onDidChangeStatus.fire(this._status);
		}
		return didHandle;
	}
	...
	private _doHandleVSBlocksSequence(data: string): boolean {
		if (!this._terminal) {
			return false;
		}
		...
		switch (command) {
			case VSBlocksOscPt.PromptStart:
				this._createOrGetCommandDetection(this._terminal).handlePromptStart();
				return true;
			case VSBlocksOscPt.CommandStart:
				this._createOrGetCommandDetection(this._terminal).handleCommandStart();
				return true;
			case VSBlocksOscPt.CommandExecuted:
				this._createOrGetCommandDetection(this._terminal).handleCommandExecuted();
				return true;
			case VSBlocksOscPt.CommandFinished: {
				const arg0 = args[0];
				const exitCode = arg0 !== undefined ? parseInt(arg0) : undefined;
				this._createOrGetCommandDetection(this._terminal).handleCommandFinished(exitCode);
				return true;
			}
			case VSBlocksOscPt.CommandLine: {
				const arg0 = args[0];
				const arg1 = args[1];
			...
			case VSBlocksOscPt.ContinuationStart: {
				this._createOrGetCommandDetection(this._terminal).handleContinuationStart();
				return true;
			}
			case VSBlocksOscPt.ContinuationEnd: {
				this._createOrGetCommandDetection(this._terminal).handleContinuationEnd();
				return true;
			}
			case VSBlocksOscPt.EnvJson: {
				const arg0 = args[0];
				const arg1 = args[1];
				if (arg0 !== undefined) {
					try {
			...
			case VSBlocksOscPt.EnvSingleEntry: {
				const arg0 = args[0];
				const arg1 = args[1];
				const arg2 = args[2];
				if (arg0 !== undefined && arg1 !== undefined) {
	...
	private _doHandleITermSequence(data: string): boolean {
		...
		switch (command) {
			...
			default: {
				...
				// Note that unlike `VSBlocksOscPt.Property`, iTerm2 does not interpret backslash or hex-escape sequences.
