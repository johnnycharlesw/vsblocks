...
export async function resolveMarketplaceHeaders(version: string,
	...
	storageService: StorageServiceInterface | undefined,
	telemetryService: ITelemetryService): Promise<IHeaders> {

	const headers: IHeaders = {
		'X-Market-Client-Id': `VSBlocks ${version}`,
		'User-Agent': `VSBlocks ${version} (${productService.nameShort})`
	};
	...
	if (supportsTelemetry(productService, environmentService) && getTelemetryLevel(configurationService) === TelemetryLevel.USAGE) {
		...
		// Send machineId as VSBlocks-SessionId so we can correlate telemetry events across different services
		// machineId can be undefined sometimes (eg: when launching from CLI), so send serviceMachineId instead otherwise
		// Marketplace will reject the request if there is no VSBlocks-SessionId header
		headers['VSBlocks-SessionId'] = telemetryService.machineId || serviceMachineId;
	}
