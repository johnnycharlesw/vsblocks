 ...
 * everything is prefixed with `--vsblocks-`.
 *
 * @sample `editorSuggestWidget.background` is `--vsblocks-editorSuggestWidget-background`.
 */
export function asCssVariableName(colorIdent: ColorIdentifier): string {
	return `--vsblocks-${colorIdent.replace(/\./g, '-')}`;
}
