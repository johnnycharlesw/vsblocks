...
suite('TelemetryLogAdapter', () => {
	...
	test('Do not Log Telemetry if log level is not trace', async () => {
		...
		const testObject = new TelemetryLogAppender('', false, testLoggerService, testInstantiationService.stub(EnvironmentServiceInterface, {}), testInstantiationService.stub(IProductService, {}));
