...
/// Number of seconds where, if there's no connections to a VSBlocks server,
...
/// served the latest version of the VSBlocks server whenever they load the
/// page. The VSBlocks server prefixes all assets and connections it loads with
/// its version string, so existing clients can continue to get served even
/// while new clients get new VSBlocks Server versions.
...
/// Gets the release info from the VSBlocks path prefix, which is in the
...
impl ConnectionManager {
	...
	/// Gets the StartData for the a version of the VSBlocks server, triggering
